...
namespace GogoGaga.OptimizedRopesAndCables
{
    ...
    public class Rope : MonoBehaviour
    {
        ...
        [Tooltip("The midpoint will not go past this position along the gravity direction.")]
        ...
        private void SetSplinePoint()
        {
            ...
            if (midPoint != null)
            {
                midPoint.position = GetRationalBezierPoint(startPoint.position, mid, endPoint.position,
                    midPointPosition, StartPointWeight, midPointWeight, EndPointWeight);
            }
        ...
        private Vector3 GetMidPoint()
        {
            ...
            Vector3 gravityAdjusted = gravityDirection.normalized * yFactor;
            midpos += gravityAdjusted;
            return midpos;
        }
        ...
        private void SimulatePhysics()
        {
            ...
            // Apply damping, acceleration, and external physics factors (e.g., wind)
            ...
            // --- Ground collision clamp respecting gravityDirection ---
            if (gravityDirection.sqrMagnitude > 0.0001f) // avoid divide-by-zero
            {
                Vector3 gDir = gravityDirection.normalized;

                // Position of midpoint along gravity axis
                float posAlongGravity = Vector3.Dot(currentValue, gDir);

                // If midpoint is "below" ground level along gravity direction
                if (posAlongGravity > groundLevel) // Change sign logic depending on your convention
                {
                    // Shift midpoint back onto ground plane
                    float diff = groundLevel - posAlongGravity;
                    currentValue += gDir * diff;

                    // Zero out velocity in gravity direction if moving further into ground
                    float velAlongGravity = Vector3.Dot(currentVelocity, gDir);
                    if (velAlongGravity > 0f)
                    {
                        currentVelocity -= gDir * velAlongGravity;
                    }
                }
            }

            // Stop simulation jitter when close enough to target

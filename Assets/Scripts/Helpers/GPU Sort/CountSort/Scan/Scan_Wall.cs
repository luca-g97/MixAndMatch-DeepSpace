...
{
    public class Scan_Wall
    {
        const int scanKernel = 0;
        const int combineKernel = 1;

        static readonly int elementsID = Shader.PropertyToID("Elements_Wall");
        static readonly int groupSumsID = Shader.PropertyToID("GroupSums_Wall");
        ...
        public Scan_Wall()
        {
            cs = ComputeHelper.LoadComputeShader("ScanTest_Wall");
        }

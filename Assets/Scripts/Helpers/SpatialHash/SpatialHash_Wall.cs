...
{
    public class SpatialHash_Wall
    {
        public ComputeBuffer SpatialKeys;
        public ComputeBuffer SpatialIndices;
        public ComputeBuffer SpatialOffsets;

        readonly GPUCountSort_Wall gpuSort = new();
        readonly SpatialOffsetCalculator_Wall spatialOffsetsCalc = new();

        public SpatialHash_Wall(int size)
        {
            CreateBuffers(size);
        }
        ...
        public void Run(int numActiveParticles)
        {
            ...
            // --- CHANGE: Pass the active count to the sorter ---
            gpuSort.Run(SpatialIndices, SpatialKeys, numActiveParticles, maxValue);

            // --- CHANGE: Pass the active count to the offset calculator ---
